  ...
  return (
    <>
        ...
        <div className="flex">
          <div className="flex-1 p-8">
            <div className="flex justify-between mb-8">
              <div>
                ...
                <div className="text-sm text-gray-600 mt-1">
                  <p>Invoice Details</p>
                  <p>Sale Date:   {data?.data?.sale_date}</p>
                  <p>Invoice No: {data?.data?.voucher_id}</p>
                </div>
              </div>
              <div className="text-right text-sm  mt-1">

              <h3 className="text-2xl font-light text-emerald-400">BILL TO</h3>
              <p className="text-gray-700">{data?.data?.customer_name}</p>
              <p className="text-gray-600">{data?.data?.customer_email}</p>
              <p className="text-gray-600">{data?.data?.customer_address}</p>
            ...
            <div className="flex justify-end mb-8">
              <div className="w-64">
                <div className="flex justify-between mb-2">
                  <span>Sub Total</span>
                  <span>{data?.data?.total} MMK</span>
                </div>
                <div className="flex justify-between mb-2">
                  <span>Tax 5%</span>
                  <span>{data?.data.tax} MMK</span>
                </div>
                <div className="flex justify-between font-semibold border-t pt-2">
                  <span>Grand Total</span>
                  <span>{data?.data.net_total} MMK</span>
                </div>
              </div>
            </div>
            ...
            <div className="mb-8">
              ...
              <p className="text-gray-600">A/c Name : {data.data.customer_name}</p>
      ...
      <div className="flex justify-end mt-4">
        <button
          onClick={handlePrint}
          className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 me-10"
        >
          Print Voucher
        </button>
        <button
